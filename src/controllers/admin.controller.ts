...
import { addOwner, fetchOwners } from "../services/admin.service";
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      ...
      res.status(500).json({
        ...
        errors: [
          "An unexpected error occurred while retrieving the owners list.",
        ],
        data: null,
      } as ErrorResponse);
    }
...
const createOwner = async (req: Request, res: Response): Promise<void> => {
  try {
    const { name, password, phone_number, site_name, site_url }: NewOwnerInput =
      req.body;

    const filePath = req.file
      ? `/uploads/images/owner_sites/${req.file.filename}`
      : null;
    ...
    if (error instanceof Error) {
      ...
      res.status(500).json({
        status: "fail",
        message: "Internal server error",
        errors: ["An unexpected error occurred while creating owner account."],
        data: null,
      } as ErrorResponse);
    }
  }
